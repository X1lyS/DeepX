...
class FofaSubdomainCollector:
    ...
    async def collect(self) -> Set[str]:
        ...
        try:
            # 执行收集
            domains = await self.collector.collect()
            self.logger.success(f"FOFA收集完成，共获取 {len(domains)} 个域名")
            return domains
        except Exception as e:
            self.logger.error(f"FOFA收集失败: {str(e)}")
            return set()
...
class DomainComparator:
    ...
    def __init__(self, target_domain: str, debug: bool = True):
        ...
        # 创建比较处理器
        self.handler = ComparisonHandler(self.logger)
    ...
    def compare(self) -> Set[str]:
        ...
        Returns:
            Set[str]: 隐藏域名集合
        """
        self.logger.model(f"隐藏资产分析模块 - 分析域名: {self.target_domain}")
        return self.handler.compare_domains()
...
class DictBruteForcer:
    ...
    async def brute_force(self) -> Set[str]:
        ...
        Returns:
            Set[str]: 爆破发现的子域名集合
        """
        self.logger.model(f"字典爆破模块 - 爆破子域名: {self.target_domain}")
